...
from core.s3_paths import build_all_key_for  # ключ формируем здесь
...
# Пикер даты и часа (без сканирования)
st.markdown("### Дата и час (S3)")
selected_date, selected_hour = render_date_hour_picker()
if not selected_date or selected_hour is None:
    st.info("Выберите день и час.")
    st.stop()

# Сформировали ключ и читаем
s3_key = build_all_key_for(selected_date, selected_hour)
try:
    df_current = normalize(read_csv_s3(s3_key))
...
# ------ Сводный график (заголовок вернули) ------ #
