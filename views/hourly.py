...
def render_hourly_mode() -> None:
    ...
    loaded_hour_starts = [
        pd.Timestamp(datetime(d.year, d.month, d.day, int(h)))
        for d, h in st.session_state.get("loaded_hours", [])
    ]
    hourly_x_range = None
    if loaded_hour_starts:
        hourly_x_range = [min(loaded_hour_starts), max(loaded_hour_starts) + pd.Timedelta(hours=1)]
    ...
    if hourly_x_range is not None:
        fig_main.update_xaxes(range=hourly_x_range)
    st.plotly_chart(
        fig_main,
